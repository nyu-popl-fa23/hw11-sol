...
object ast:
  ...
  sealed abstract class Expr extends Positional:
    ...
    // clone the expression (shallow copy) and update position
    def clone(pos: scala.util.parsing.input.Position): Expr = 
      val e = this match
        case e@Var(_) => e.copy()
        case e@UnOp(_, _) => e.copy()
        case e@BinOp(_, _, _) => e.copy()
        case e@Decl(_, _, _, _) => e.copy()
        case e@Function(_, _, _, _) => e.copy()
        case e@Call(_, _) => e.copy()
        case e@ObjLit(_) => e.copy()
        case e@Addr(_) => e.copy()
        case e@Num(_) => e.copy()
        case e@Bool(_) => e.copy()
        case e@Str(_) => e.copy()
        case e@Undefined => e
        case e@If(_, _, _) => e.copy()
        case e@Print(_) => e.copy()
      e.pos = pos
      e
  ...
  /* Memory contents */
  sealed abstract trait Con extends Positional
